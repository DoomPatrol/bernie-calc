...
class CalculatorResultsView(View):
  ...
  def post(self, request):
    ...
    context_dict['overall_difference'] = calculate_overall_difference(yearly_income, context_dict['total_yearly_spending'], context_dict['current_taxes'], context_dict['medicare_for_all_taxes'])
    context_dict['healthcare_difference'] = calculate_healthcare_difference(yearly_income, total_healthcare_spending, context_dict['current_taxes'], context_dict['medicare_for_all_taxes'])

    # let's create some friendly objects we can use for data viz
    context_dict['bernie_monthly_income_doughnut_data'] = {
      'monthly_taxes': round(context_dict['medicare_for_all_taxes']['breakdown']['total_taxes'] /12 , 2),
